...
Nomio.World 高度重视用户隐私。完整的隐私政策详见 [PrivacyView.vue](dashboard/src/views/PrivacyView.vue) 组件，以下为核心要点摘要。
...
|------|------|
| 最小化收集 | 仅收集服务运行所必需的信息 |
| 目的限定 | 数据仅用于提供服务、安全防护与服务改进 |
| 不出售数据 | 绝不出售、出租或交易用户数据 |
| 不追踪 | 不使用第三方追踪 Cookie，不参与广告网络 |

### 8.3 收集的数据

- **用户主动提供：** 用户名、密码（PBKDF2 加密）、源站地址、转发邮箱
- **自动收集：** IP 地址、请求日志、邮件元数据（发件人、主题、时间）
- **不收集：** 实名信息、支付信息、精确位置、设备指纹

### 8.4 数据安全措施

- 全链路 HTTPS 加密传输
- PBKDF2 (SHA-256, 100k 迭代) 密码哈希
...
用户享有访问权、更正权、删除权、导出权和撤回同意权。详见隐私政策全文。

### 8.6 MSIX 打包应用声明

本应用的桌面版本由传统 Win32 .exe 程序通过 MSIX Packaging Tool 打包而成。为保证原有功能（文件系统访问、注册表读写等）正常运行，打包清单中声明了 `runFullTrust` 受限功能。此声明仅用于维持应用原有功能，不用于任何超出正常运行所需的系统操作。
