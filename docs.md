...
Namio.World 是一个基于 Cloudflare 边缘计算平台的公益数字身份基建服务，为用户提供两大核心能力：

- **二级域名分发：** 用户可免费申请 `username.namio.world` 格式的二级域名，通过 Worker 网关反向代理到用户自托管的 HTTPS 源站。
- **邮箱托管服务：** 每个用户自动获得 `username@namio.world` 邮箱地址，支持纯文本邮件接收与存储，暂不支持邮件发送和附件。
...
Namio 为生造词，取自 **Name + I/O**（名称的输入与输出），寓意"域名与邮箱的入口服务"。`.world` 后缀强调全球接入属性。
...
┌─────────────────────────────────────────────────────────────────┐
│                         终端用户                                │
│    访问 https://username.namio.world 或 发送邮件至             │
│              username@namio.world                              │
└──────────────────────────┬──────────────────────────────────────┘
                           │
                           ▼
┌─────────────────────────────────────────────────────────────────┐
│                    Cloudflare 边缘网络                          │
├─────────────────────────────────────────────────────────────────┤
│  DNS: *.namio.world (泛域名, 橙色云开启)                       │
│  Email Routing: *@namio.world → Email Worker                   │
├─────────────────────────────────────────────────────────────────┤
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐ │
...
用户浏览器                    Cloudflare 边缘                     用户源站
    │                              │                                 │
    │  GET alice.namio.world       │                                 │
    │─────────────────────────────>│                                 │
    │                              │  1. DNS 解析 *.namio.world      │
    │                              │  2. 匹配 Worker 路由            │
    │                              │  3. 查询 D1 获取源站地址        │
    │                              │                                 │
...
| 记录类型 | 名称 | 内容 | 代理状态 |
|----------|------|------|----------|
| A | `*.namio.world` | `192.0.2.1`（占位） | 橙色云（Proxied） |
| AAAA | `*.namio.world` | `100::`（占位） | 橙色云（Proxied） |
| MX | `namio.world` | `route.mx.cloudflare.net` | DNS only |
...
所有发送至 `@namio.world` 的邮件，均以 HTTP 请求形式转发给 Email Worker 处理。
...
| 单封邮件收件人数 | > 1 个 `@namio.world` 用户 | 仅投递第一个，忽略其余 |
...
- **反垃圾邮件：** 自建 SMTP 若被滥用，`namio.world` 域名将被全球邮件服务商拉黑，连带影响网页服务信誉。
- **合规简化：** 不存储用户发送的邮件，降低数据合规压力。
- **成本控制：** 发信需对接第三方 API（如 SendGrid）或自建 SMTP 服务器，增加运维成本。

> 用户如需用 `@namio.world` 地址回复邮件，可在第三方邮箱客户端（如 Gmail）中设置"以该地址代发"，但平台不承担因此产生的域名信誉风险。
...
| 字段 | 说明 | 必填 |
|------|------|------|
| 邮箱地址 | 自动生成 `用户名@namio.world`，不可修改 | 是 |
...
| 层级 | 措施 |
|------|------|
| 用户 ↔ Cloudflare | 强制 HTTPS，Cloudflare 边缘证书自动覆盖 `*.namio.world` |
...
2. 用户需在源站部署文件 `{origin_url}/.well-known/namio-verify.txt`，内容为该 Token
...
```
https://myapp.vercel.app/.well-known/namio-verify.txt
内容: namio-verify=a1b2c3d4e5f6...
```

此机制可防止恶意用户将 `bank.namio.world` 指向钓鱼网站。
...
| 措施 | 说明 |
|------|------|
| CORS 策略 | Hono `cors()` 中间件，仅允许 `https://namio.world` 来源 |
...
# 2. 创建 D1 数据库
wrangler d1 create namio-db
# 记录输出的 database_id，填入各 wrangler.toml

# 3. 初始化数据库表
wrangler d1 execute namio-db --file=./schema.sql
...
| 类型 | 名称 | 内容 | 代理 |
|------|------|------|------|
| A | `*.namio.world` | `192.0.2.1`（占位） | 橙色云 |
| AAAA | `*.namio.world` | `100::`（占位） | 橙色云 |
| MX | `namio.world` | `route.mx.cloudflare.net` | DNS only |

**Email Routing：**
1. 进入 Cloudflare Dashboard → Email Routing
2. 添加 Catch-all 规则：`*@namio.world` → Send to Worker → `namio-email`
...
routes = [
  { pattern = "*.namio.world/*", zone_name = "namio.world" }
]

[[d1_databases]]
binding = "DB"
database_name = "namio-db"
...
routes = [
  { pattern = "namio.world/api/*", zone_name = "namio.world" }
]
...
1. **注册域名：** 在 Cloudflare 注册或转入 `namio.world`
2. **DNS 配置：**
   - 添加 `*.namio.world` 泛域名 A/AAAA 记录，开启橙色云
   - 添加 `namio.world` MX 记录指向 `route.mx.cloudflare.net`
3. **创建 D1 数据库：**
   ```bash
   npx wrangler d1 create namio-db
   # 输出 database_id，填入各 wrangler.toml
   ```
4. **初始化数据库表：**
   ```bash
   npx wrangler d1 execute namio-db --file=./schema.sql
   ```
5. **安装依赖并部署 Workers：**
   ```bash
...
# 查看 D1 数据库内容
npx wrangler d1 execute namio-db --command "SELECT * FROM users"
```
...
| 无自定义域名 | 仅支持 `*.namio.world` | 未来可扩展支持绑定自有域名 |
...
| 术语 | 说明 |
|------|------|
| 泛域名解析 | 使用 `*.namio.world` 一条记录覆盖所有子域名 |
